...
class LoginFrame(tk.Frame):
	def __init__(self, parent, controller):
		...
		self.titleLabel = tk.Label(self.loginFrame, text='Password Manager', bg = self.backgroundColor, fg = self.primaryColor, font=("Rockwell", 18, "bold"))
		self.titleLabel.place(relx=0.25, rely=0.2, relheight=0.1, relwidth=0.5)
		self.epassLabel = tk.Label(self.loginFrame, text='Master Password', bg = self.backgroundColor, fg = self.secTextColor, font=self.labelFont)
	...
	# Shortcut for Enter key - enter key triggers input check
