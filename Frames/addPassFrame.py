...
class AddPassFrame(tk.Frame):
	def __init__(self, parent, controller):
		...
		self.secTextColor = '#fffaaa'

		self.sd = siteData()
		# frame components
		self.addPassFrame = tk.LabelFrame(self, text="Add New Password", bd=5, bg = self.backgroundColor, fg = self.priTextColor)
		...
		self.siteLabel = tk.Label(self.addPassFrame, text='Enter the website name', bg = self.backgroundColor, fg = self.priTextColor, font = self.labelFont)
		self.siteLabel.place(relx=0.25, rely=0.22, relheight=0.07, relwidth=0.5)
		self.siteText = tk.Entry(self.addPassFrame, width=25, font=self.entryFont, bg = self.surface1Color, fg = self.priTextColor)
		self.siteText.place(relx=0.25, rely=0.29, relheight=0.05, relwidth=0.5)
		self.usernameLabel = tk.Label(self.addPassFrame, text='Enter the username', bg = self.backgroundColor, fg = self.priTextColor, font = self.labelFont)
		self.usernameLabel.place(relx=0.25, rely=0.38, relheight=0.07, relwidth=0.5)
		self.usernameText = tk.Entry(self.addPassFrame, width=25, font=self.entryFont, bg = self.surface1Color, fg = self.priTextColor)
		self.usernameText.place(relx=0.25, rely=0.44, relheight=0.05, relwidth=0.5)
		self.passLabel = tk.Label(self.addPassFrame, text='Enter/ generate password', bg = self.backgroundColor, fg = self.priTextColor, font = self.labelFont)
		self.passLabel.place(relx=0.25, rely=0.52, relheight=0.07, relwidth=0.5)
		self.passText = tk.Entry(self.addPassFrame, width=25, font=self.entryFont, bg = self.surface1Color, fg = self.priTextColor)
...
# calls generatePass method from Backend to generate password based on set rules
...
# saves the generated site details (credentials) into the database
	def savePass(self):
		if not self.siteText.get() or not self.usernameText.get() or not self.passText.get():
			saveErrorLabel = tk.Label(self.addPassFrame, text = "Empty fields!", bg=self.errorColor, fg=self.secTextColor, font = self.labelFont)
			saveErrorLabel.place(relx=0.16, rely=0.02, relwidth=0.7, relheight=0.05)
			saveErrorLabel.after(2000, saveErrorLabel.destroy)
			return
